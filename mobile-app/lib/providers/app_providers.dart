...
import '../core/location/punch_location.dart';
...
/// Reads the phone's position for a punch.
///
/// A provider rather than a plain constructor call so a test can substitute one
/// that answers instantly — the real one talks to the platform, which in a test
/// environment means waiting for a channel that will never reply.
final Provider<PunchLocationService> punchLocationServiceProvider =
    Provider<PunchLocationService>((ref) => const PunchLocationService());

final Provider<FaceRepository> faceRepositoryProvider =
    Provider<FaceRepository>((ref) => FaceRepository(ref.watch(apiClientProvider)));
